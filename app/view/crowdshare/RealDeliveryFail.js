Ext.define('JxkpApp.view.crowdshare.RealDeliveryFail' ,{
    extend: 'Ext.grid.Panel', 
    alias : 'widget.realdeliveryfail', //使用alias来定义一个别名，这个时候，我们的类可以使用Ext.create()和Ext.widget()创建，在其他组件的子组件中，也可以使用xtype来创建。
    title : '实交未妥投邮件', 
                                        ...
                                        specialkey: function(field,e){
                                            ...
                                            if (e.getKey()==Ext.EventObject.ENTER){ 
                                                ...
                                                if(index == -1){
                                                    ...
                                                    if(rec.get('DELIVERYOK') == 1){
                                                        rec.set('HANDBACK', true);
                                                        // rec.commit();
                                                        store.getProxy().setExtraParam('type', 'handback');
                                                        store.sync();
                                                    }else{
                                                        Ext.Msg.alert('info', '该邮件登记为妥投邮件，请检查！', function(){
                                                                field.setValue('');
                                                                field.focus(false, 200);                                                                
                                                        });
                                                    }
